...
for b in branches:
    print(f"Processing {b['branch']}...")
    if b['message'] != 'amend':
        subprocess.run(["git", "checkout", "master"])
        subprocess.run(["git", "pull", "origin", "master"])
        subprocess.run(["git", "branch", "-D", b["branch"]], stderr=subprocess.DEVNULL)
        subprocess.run(["git", "checkout", "-b", b["branch"]])
    ...
    if b["search"] in content:
        ...
        with open(b["file"], 'w', encoding='utf-8') as f:
            f.write(content)
        subprocess.run(["git", "add", b["file"]])
        if b['message'] == 'amend':
            subprocess.run(["git", "commit", "--amend", "--no-edit"])
        else:
            subprocess.run(["git", "commit", "-m", b["message"]])
        print(f"Committed {b['branch']}!")
    else:
        print(f"Search string not found in {b['file']}")
