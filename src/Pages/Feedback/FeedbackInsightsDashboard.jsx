...
export default function FeedbackInsightsDashboard() {
  ...
  const trendData = useMemo(() => {
    const byDate = {};
    feedback.forEach((f) => {
      const date = new Date(f.submittedAt).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
      });
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Post-event attendee satisfaction analytics
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div className="rounded-xl border border-gray-100 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          ...
          <p className="text-xs text-gray-400">
            {recStats.recommendCount} of {recStats.total}
          </p>
        </div>
        <div className="rounded-xl border border-gray-100 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <div className="mb-1 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-100 bg-white p-5 dark:border-gray-800 dark:bg-gray-900">
          <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-gray-300">
            <TrendingUp className="h-4 w-4 text-indigo-500" /> Feedback Trend (Last 7 Days)
          </h2>
          <ResponsiveContainer width="100%" height={220}>
