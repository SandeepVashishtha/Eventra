...
const EventCard = ({ event }) => {
  ...
  const capacityInfo = useMemo(() => {
    ...
    return {
      ...
      registered,
      spotsLeft,
      ...getCapacityStyles(ratio, isFull),
    };
  ...
  const handleCopyLink = useCallback(
    (e) => {
      ...
      navigator.clipboard
        .writeText(shareUrl)
        .then(() => toast.success("Event link copied to clipboard!", { autoClose: 2000 }))
        .catch(() => toast.error("Could not copy link. Please try again.", { autoClose: 2500 }));
    },
    [event.id],
  );
  ...
  const handleBookmarkToggle = useCallback(
    (e) => {
      ...
      if (isBookmarked) {
        ...
        toast.info("Removed from bookmarked events.", { autoClose: 1800 });
        return;
      }

      addBookmarkedEvent({ ...event, status: computedStatus });
      setIsBookmarked(true);
      toast.success("Event bookmarked.", { autoClose: 1800 });
    },
  ...
  return (
    ...
    >
      <div className="absolute right-3 top-[5.5rem] z-[200] flex space-x-1.5">
        ...
        >
          {isBookmarked ? <BookmarkCheck size={14} fill="currentColor" aria-hidden="true" /> : <Bookmark size={14} aria-hidden="true" />}
        </button>

        <ShareMenu shareData={eventSharingData} position="above" menuClassName="!z-[999] shadow-2xl" buttonClassName="">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-gray-600" aria-hidden="true">
            <rect width="14" height="14" x="8" y="8" rx="2" ry="2" />
            <path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" />
          </svg>
        </button>

        <a href={addEventToGoogleCalendar(event)} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} title="Add to Google Calendar" aria-label={`Add ${event.title} to Google Calendar`} className="rounded-full focus-visible:ring-2 focus-visible:ring-indigo-500">
          <div className="rounded-full border border-gray-200 bg-white/90 p-2 shadow backdrop-blur-sm">
      ...
      <div className="flex items-center gap-4 border-b border-gray-100 bg-gradient-to-r from-white/80 to-indigo-50/60 px-5 py-4 dark:border-gray-800 dark:from-gray-900/80 dark:to-indigo-950/60">
        <div className="shrink-0 rounded-xl bg-gradient-to-br from-gray-100 to-white p-2 shadow-inner dark:from-gray-800 dark:to-gray-700">{randomIcon}</div>
        <h3 id={titleId} className="flex-1 truncate text-lg font-bold tracking-tight text-gray-900 transition-colors duration-300 group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-400">{event.title}</h3>
        <div className="ml-auto"><StatusBadge status={computedStatus} /></div>
      </div>

      <div className="relative aspect-video overflow-hidden bg-gray-100 dark:bg-gray-800">
        <LazyImage loading="lazy" decoding="async" src={event.image} alt={event.imageAlt || `${event.title} event thumbnail`} width={640} height={360} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent" />
      </div>

      <div className="border-b border-gray-100 px-5 py-4 dark:border-gray-800">
        <p className="line-clamp-2 text-sm leading-6 text-gray-500 dark:text-gray-400">{event.description}</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-x-4 gap-y-3 bg-gray-50/50 px-5 py-4 text-sm text-gray-600 dark:bg-gray-800/30 dark:text-gray-400 xs:grid-cols-2">
        <div className="flex min-w-0 items-center gap-2"><MapPin size={14} className="shrink-0 text-pink-500" aria-hidden="true" /><span className="truncate">{event.location}</span></div>
        <div className="flex min-w-0 items-center gap-2"><Clock size={14} className="shrink-0 text-blue-500" aria-hidden="true" /><span className="truncate">{event.time}</span></div>
        <div className="flex min-w-0 items-center gap-2"><Tag size={14} className="shrink-0 text-green-500" aria-hidden="true" /><span className="truncate">{event.type}</span></div>
        <div className="flex min-w-0 items-center gap-2"><Calendar size={14} className="shrink-0 text-indigo-500" aria-hidden="true" /><span className="truncate">{formattedDate}</span></div>
      </div>
      ...
      {capacityInfo && (
        <div className="border-t border-gray-100 bg-gray-50/50 px-5 py-3 dark:border-gray-800 dark:bg-gray-800/30">
          <div className="mb-1.5 flex items-center justify-between">
            ...
            {capacityInfo.isFull ? <span className="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold text-red-700 dark:bg-red-900/40 dark:text-red-300">Full</span> : <span className={`text-xs font-semibold tabular-nums ${capacityInfo.textColor}`}>{capacityInfo.spotsLeft} spot{capacityInfo.spotsLeft === 1 ? "" : "s"} left</span>}
          </div>
          <div className="h-1.5 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-gray-700" role="progressbar" aria-valuenow={capacityInfo.percent} aria-valuemin={0} aria-valuemax={100} aria-label={`${capacityInfo.registered} of ${capacityInfo.capacity} seats filled`}>
            <div className={`h-full ${capacityInfo.barColor} transition-all duration-500 ease-out`} style={{ width: `${capacityInfo.percent}%` }} />
          </div>
          <div className="mt-1 text-[11px] text-gray-500 dark:text-gray-500 tabular-nums">{capacityInfo.registered} / {capacityInfo.capacity} registered</div>
        </div>
      )}

      <div className="mt-auto flex gap-3 px-5 py-4">
        {isPastEvent ? <div className="inline-flex min-h-[44px] flex-1 cursor-not-allowed items-center justify-center rounded-2xl bg-gray-300 px-4 py-3 text-sm font-semibold text-gray-600 shadow-md dark:bg-gray-700 dark:text-gray-300">Event Ended</div> : <Link to={`/events/${event.id}/register`} className="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-600 via-indigo-700 to-slate-900 px-4 py-3 text-sm font-semibold text-white shadow-lg transition-all duration-300 hover:from-indigo-500 hover:via-indigo-600 hover:to-slate-800 hover:shadow-xl sm:hover:scale-[1.03]">Register Now</Link>}

        <Link to={`/events/${event.id}`} className="inline-flex min-h-[44px] flex-1 items-center justify-center rounded-2xl border border-indigo-200 bg-white/80 px-4 py-3 text-sm font-semibold text-indigo-700 shadow-md transition-all duration-300 hover:bg-indigo-50 hover:text-indigo-800 hover:shadow-lg dark:border-indigo-700 dark:bg-gray-800 dark:text-indigo-300 dark:hover:bg-indigo-900/30 dark:hover:text-white sm:hover:scale-[1.03]" aria-label={`View details for ${event.title}`}>View Details</Link>
      </div>
    </article>
  );
};
