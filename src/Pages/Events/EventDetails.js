...
const fetchRegistrantsForExport = async (eventId) => {
  const getPage = async (page) => {
    const res = await apiUtils.get(`${API_ENDPOINTS.EVENTS.REGISTRANTS(eventId)}?page=${page}&limit=500`);
    return { data: res.data?.data || res.data || [], totalPages: res.data?.totalPages || 1 };
  };
  ...
  while (hasMore) {
    const { data, totalPages } = await getPage(page);
    if (Array.isArray(data)) allRegistrants.push(...data);
...
const getDraftLocation = (loc) => {
  const isStr = typeof loc === "string";
  return {
    name: isStr ? loc : (loc?.name || ""),
    address: isStr ? "" : (loc?.address || ""),
    coordinates: {
      latitude: isStr ? "" : (loc?.coordinates?.latitude ?? ""),
      longitude: isStr ? "" : (loc?.coordinates?.longitude ?? ""),
    },
  };
};

const getDraftTiers = (tiers) => {
...
const getDraftDates = (evt) => {
  ...
  const endD = evt.endDate || evt.date || evt.startDate;
  const pStart = parseD(startD);
  const pEnd = parseD(endD);
  const isMulti = pStart && pEnd && pStart !== pEnd;
  ...
  return {
    ...
    startTime: parseT(startD),
    endTime: parseT(endD),
    registrationStart: parseD(evt.registrationStart),
    registrationEnd: parseD(evt.registrationEnd),
  };
};
