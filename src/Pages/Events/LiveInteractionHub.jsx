...
const LiveInteractionHub = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-900 via-purple-900 to-black p-6">
      <div className="mx-auto max-w-4xl rounded-3xl border border-white/20 bg-white/10 shadow-xl backdrop-blur-xl">
        <Tab.Group selectedIndex={selectedIndex} onChange={setSelectedIndex}>
          <Tab.List className="flex space-x-2 p-2">
            {tabs.map((tab) => (
              <Tab
                key={tab}
                className={({ selected }) =>
                  `w-full rounded-lg py-2.5 text-sm font-medium text-white transition-colors duration-200 focus:outline-none ${selected ? "bg-indigo-600" : "bg-white/20 hover:bg-white/30"}`
                }
              >
                {tab}
              </Tab>
