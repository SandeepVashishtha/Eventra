    ...
    try {
      ...
      const normalizedProjects = projectsList.map((p) => ({
        ...
        techStack: Array.isArray(p.techStack)
          ? p.techStack.map((t) => (typeof t === "string" ? t : (t?.name ?? t?.label ?? String(t))))
          : [],
        author:
          typeof p.author === "string"
            ? p.author
            : (p.author?.name ?? p.author?.username ?? "Anonymous"),
        status: typeof p.status === "string" ? p.status : (p.status?.name ?? "Active"),
        difficulty:
          typeof p.difficulty === "string" ? p.difficulty : (p.difficulty?.name ?? "Intermediate"),
        category:
          typeof p.category === "string"
            ? p.category
            : (p.category?.name ?? p.category?.label ?? "General"),
        githubUrl:
          typeof p.githubUrl === "string" ? p.githubUrl : (p.githubUrl?.url ?? p.repoUrl ?? ""),
        liveDemo: typeof p.liveDemo === "string" ? p.liveDemo : (p.liveDemo?.url ?? ""),
      }));
