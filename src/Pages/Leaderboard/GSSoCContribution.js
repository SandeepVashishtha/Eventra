...
const HeroStatCard = memo(({ label, value, icon: Icon }) => (
  ...
  >
    <Icon
      className="w-4 h-4 sm:w-5 sm:h-5 mx-auto mb-1.5 text-indigo-500 dark:text-indigo-400"
      aria-hidden="true"
    />
    <div className="text-lg sm:text-xl font-bold tabular-nums text-gray-900 dark:text-white">
      {formatNumber(value)}
    </div>
...
const HorizontalTimeline = memo(({ timeline, variants }) => {
  ...
  return (
    <motion.section
      variants={variants}
      className="p-5 sm:p-8 rounded-3xl bg-white dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60 shadow-lg shadow-slate-200/30 dark:shadow-none mb-6 sm:mb-8 backdrop-blur-xl"
    >
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 mb-10 border-b border-slate-100 dark:border-slate-700/50 pb-6">
        <div>
          ...
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400 ml-[52px]">
            Track your milestones and upcoming deadlines
          </p>
        </div>
        ...
        <div className="flex flex-col md:items-end bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800/80 shadow-inner">
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <span className="text-xs font-black text-white bg-gradient-to-r from-indigo-500 to-violet-600 px-2.5 py-1 rounded-full shadow-md shadow-indigo-500/20">
      ...
      <div className="relative w-full py-4 overflow-x-auto hide-scrollbar">
        <div className="min-w-[700px] flex items-start justify-between relative px-8 md:px-12 pb-8 pt-2">
          <div
            className="absolute top-[34px] left-20 right-20 h-2 bg-slate-100 dark:bg-slate-800/80 rounded-full overflow-hidden border border-slate-200/50 dark:border-slate-700/50"
            aria-hidden="true"
          >
            <motion.div
              initial={{ width: 0 }}
          ...
          {timeline.map((item, idx) => {
            ...
            return (
              <div
                key={item.phase}
                className="relative flex flex-col items-center w-32 shrink-0 z-10 group"
                role="listitem"
              >
                <motion.div
                  initial={{ scale: 0 }}
                  ...
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center border-4 shadow-xl transition-all duration-300 ${
                    isCompleted
                      ? "bg-gradient-to-br from-indigo-500 to-violet-600 border-white dark:border-slate-900 text-white group-hover:scale-110 group-hover:-translate-y-1 shadow-indigo-500/20"
                      : isCurrent
                        ? "bg-white dark:bg-slate-900 text-indigo-600 dark:text-indigo-400 border-indigo-500 shadow-indigo-500/30 scale-110 group-hover:scale-125 group-hover:-translate-y-1"
                ...
                <div className="mt-5 text-center">
                  <h4
                    className={`text-sm font-extrabold mb-1.5 transition-colors ${
                      isCurrent
                  ...
                  <p
                    className={`text-xs ${isCurrent ? "text-slate-700 dark:text-slate-300 font-bold" : "text-slate-500 dark:text-slate-500 font-medium"}`}
                  >
                    {item.date}
                  </p>
                </div>
        ...
        >
          ...
          <div className="relative z-10 grid md:grid-cols-2 gap-6 sm:gap-8 items-center">
            <div>
              ...
              <h1
                id="hero-heading"
                className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 sm:mb-4 tracking-tight leading-tight text-gray-900 dark:text-white"
              >
            ...
            >
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                ...
                <h3 id="countdown-heading" className="font-semibold text-gray-900 dark:text-white">
                  Program Ends In
                </h3>
              </div>
        ...
        >
          <div className="text-center mb-6 sm:mb-8">
            <h2
              ...
              className="text-xl sm:text-2xl font-bold text-indigo-700 dark:text-indigo-400 mb-2"
            >
