...
/**
 * Helper to safely parse and validate a date value.
 * Returns a valid Date object or null if invalid/missing.
 */
const parseValidDate = (val) => {
  if (val === null || val === undefined || typeof val === "boolean" || Array.isArray(val)) {
    return null;
  }
...
/**
 * Calculates completion progress percentage (0-100) for a milestone based on dates.
 * Safely handles missing, null, or invalid milestone data and date formats.
 */
export const calculateMilestoneProgress = (milestone, currentDate = new Date()) => {
  if (!milestone || typeof milestone !== "object") {
    return 0;
  }

  const start = parseValidDate(milestone.startDate);
  const end = parseValidDate(milestone.endDate);
  ...
  const current = parseValidDate(currentDate) || new Date();
