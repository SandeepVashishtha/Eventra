...
import { Mail, Lock, ArrowRight, CheckCircle2, AlertCircle, Clock } from "lucide-react";
...
function LoginForm() {
  ...
  if (success) {
    return (
      <div className="py-8 text-center space-y-3 bg-emerald-50 border border-emerald-200 rounded-2xl p-6">
        ...
        <h3 className="text-lg font-bold text-zinc-900">Login Successful!</h3>
  ...
  return (
    <div className="space-y-4">
      {sessionExpiredMsg && (
        <div className="p-3.5 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl text-xs flex items-center gap-2.5 font-medium shadow-sm">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          <span>{loading ? "Signing in..." : "Sign In to Eventra"}</span>
      ...
      <div className="w-full max-w-md bg-white border border-emerald-900/10 rounded-3xl p-8 shadow-xl space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-xs text-zinc-500 max-w-xs mx-auto">
            Sign in to access your registered events and user dashboard.
          </p>
        </div>

        <Suspense fallback={<div className="text-center text-xs text-zinc-400 py-4">Loading login form...</div>}>
          <LoginForm />
        </Suspense>

        <div className="pt-4 text-center text-xs text-zinc-500 border-t border-zinc-100">
          Don&apos;t have an account?{" "}
          <Link href="/signup" className="font-bold text-zinc-900 hover:text-[#00b887] transition-colors">
            Create one now
          </Link>
        </div>
