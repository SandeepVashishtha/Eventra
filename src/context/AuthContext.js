...
export const AuthProvider = ({ children }) => {
  ...
  const clearExpiredSessionRef = useRef(null);
  ...
  const clearExpiredSession = useCallback(() => {
    logger.warn('[AuthContext] Session expiration detected. Clearing session state immediately.');
    clearSession();
  ...
  const getScopesForRoles = useCallback((roles = []) => {
  ...
  const extractSession = useCallback(
    (res, data, fallbackEmail) => {
      ...
      const sessionUser = {
        ...
        roles: resolvedRoles,
        permissions,
        scopes: rawUser?.scopes ?? getScopesForRoles(resolvedRoles),
      };

      return { sessionToken, sessionUser };
    },
    [getScopesForRoles, normalizeRoles]
  );
  ...
  const login = useCallback(
    ...
    [extractSession, getAuthErrorMessage, persistSession, setAuthRequestState]
  );
