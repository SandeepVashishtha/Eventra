  ...
  return (
    <div className="header-branding-wrapper mb-6 text-center select-none">
      ...
      <p className="mx-auto mt-1 max-w-xs text-xs text-gray-500 dark:text-gray-400">
        Join the Eventra network portal to securely manage, map, and organize live interactive
        project hub environments.
      </p>
    </div>
  );
});
...
const FormFieldIconSelector = memo(({ fieldName }) => {
  const iconClasses =
    "w-4 h-4 text-gray-400 dark:text-gray-500 transition-colors duration-200 group-focus-within:text-blue-500";
...
const SignupFormExample = ({ onSignupSuccess }) => {
  ...
  const validateUsernameAvailable = useCallback(async (username) => {
    ...
    return !takenUsernames.includes(username.toLowerCase()) || "Username already taken";
  }, []);
  ...
  const validateEmailAvailable = useCallback(async (email) => {
    ...
    const registeredEmails = ["test@example.com", "admin@example.com", "demo@test.com"];
    return !registeredEmails.includes(email.toLowerCase()) || "Email already registered";
  }, []);
  ...
  const {
    ...
    handleSubmit,
    resetForm,
  } = useFormValidation(INITIAL_FORM_VALUES, validationRules, DEBOUNCE_CONFIGURATION_PRESETS);
  ...
  const handleFormSubmit = useMemo(
    () =>
      handleSubmit(async (formValues) => {
        ...
        try {
          logger.info(
            "Form validation pipeline passed. Submitting credentials payload:",
            formValues
          );
      ...
      <form onSubmit={handleFormSubmit} className="space-y-4" noValidate>
        ...
        <div className="action-submission-wrapper pt-2">
          <motion.button
            ...
            className={`w-full rounded-xl py-3 text-sm font-bold tracking-wide shadow-md transition-all focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:outline-none ${
              !isSubmitting
                ? "cursor-pointer bg-blue-600 text-white shadow-blue-500/10 hover:bg-blue-700"
  ...
  ({
    ...
    return (
      <div className="field-structural-node-block group space-y-1.5">
        ...
        <div
          className={`flex w-full items-center gap-3 rounded-xl border-2 bg-white px-3.5 py-2 shadow-sm transition-all duration-200 focus-within:ring-2 focus-within:ring-blue-500/20 dark:bg-gray-800 ${fieldBorderClasses} `}
        >
