...
const assessStrength = (password) => {
  const criteria = [
    ...
    { label: "Contains lowercase letter", met: password ? /[a-z]/.test(password) : false },
    {
      ...
      met: password ? /[!@#$%^&*(),.?":{}|<>]/.test(password) : false,
    },
  ];
      ...
      {password && (
        ...
        >
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            <span
              className={`font-black ${strengthColorClass} rounded-md border border-slate-200/20 bg-slate-100/50 px-2 py-0.5 dark:bg-slate-800/40`}
            >
          ...
          <motion.p
            className={`mt-2 text-[11px] font-medium ${strengthColorClass} leading-relaxed`}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <ul className="mt-4 grid grid-cols-1 gap-2 border-t border-slate-200/40 pt-3 sm:grid-cols-2 dark:border-slate-800/40">
            {criteria.map((c, index) => (
              <li
                key={index}
                className={`flex items-center gap-2 rounded-xl border px-3 py-1.5 text-[11px] transition-all duration-300 ${
                  c.met
                    ? "border-green-500/20 bg-green-500/5 text-green-700 shadow-sm shadow-green-500/5 dark:text-green-400"
                    : "border-slate-100/50 bg-slate-100/30 text-slate-400 dark:border-slate-800/50 dark:bg-slate-800/20 dark:text-slate-500"
                }`}
              >
                <span
