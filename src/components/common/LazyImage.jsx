import { useState, useRef, useEffect } from 'react';
import { useInView } from 'react-intersection-observer';
import { ImageIcon } from 'lucide-react';
import useReducedMotion from '../../hooks/useReducedMotion';
import '../../styles/lazy-image.css';

const LazyImage = ({
  src,
  alt = '',
  className = '',
  imgClassName = '',
  width,
  height,
  aspectRatio,
  loading = 'lazy',
  decoding = 'async',
  style,
  imgStyle,
  useWebP = false,
  onError,
  previewSrc,
  rootMargin = '200px',
  ...props
}) => {
  const [loaded, setLoaded] = useState(false);
  const [error, setError] = useState(false);
  const imgRef = useRef(null);
  const prefersReducedMotion = useReducedMotion();

  const { ref: inViewRef, inView } = useInView({
    triggerOnce: true,
    rootMargin,
  });

  useEffect(() => {
    setLoaded(false);
    setError(false);
  }, [src]);

  useEffect(() => {
    if (inView && imgRef.current?.complete && imgRef.current.naturalWidth > 0) {
      setLoaded(true);
    }
  }, [src, inView]);

  const webpSrc =
    useWebP && src && src.match(/\.(jpe?g|png)$/i)
      ? src.replace(/\.(jpe?g|png)$/i, ".webp")
      : null;

  const containerStyle = {
    position: "relative",
    overflow: "hidden",
    ...style,
  };

  if (width !== undefined) {
    containerStyle.width =
      typeof width === "number" ? `${width}px` : width;
  }

  if (height !== undefined) {
    containerStyle.height =
      typeof height === "number" ? `${height}px` : height;
  }

  if (aspectRatio) {
    containerStyle.aspectRatio = aspectRatio;
  }

  const handleOnError = (e) => {
    setError(true);
    onError?.(e);

    e.target.onerror = null;
    e.target.src =
      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="800" height="400" fill="%23f3f4f6"><rect width="100%" height="100%"/><text x="50%" y="50%" fill="%239ca3af" font-family="sans-serif" font-size="24" text-anchor="middle" dominant-baseline="middle">Image Not Available</text></svg>';
  };

  const showImage = inView && !error;

  const transitionStyle = prefersReducedMotion
    ? { transition: 'none' }
    : {};

  const imgElement = showImage ? (
    <img
      ref={imgRef}
      src={src}
      alt={alt}
      loading={loading}
      decoding={decoding}
      onLoad={() => setLoaded(true)}
      onError={handleOnError}
      className={`lazy-img ${
        loaded ? "lazy-img--loaded" : "lazy-img--loading"
      } ${imgClassName || className || ""}`}
      style={{
        width: "100%",
        height: "100%",
        display: "block",
        ...transitionStyle,
        ...imgStyle,
      }}
      {...props}
    />
  ) : null;

  return (
    <div ref={inViewRef} className={`lazy-img-container ${className}`} style={containerStyle}>
      {!loaded && <div className="lazy-img-skeleton" />}

      {!loaded && previewSrc && (
        <img
          src={previewSrc}
          alt=""
          aria-hidden="true"
          className="lazy-img-preview"
        />
      )}

      {error && (
        <div className="lazy-img-error">
          <ImageIcon className="lazy-img-error-icon" size={32} />
        </div>
      )}

      {webpSrc && showImage ? (
        <picture>
          <source srcSet={webpSrc} type="image/webp" />
          {imgElement}
        </picture>
      ) : (
        imgElement
      )}
    </div>
  );
};

export default LazyImage;
