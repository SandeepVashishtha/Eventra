...
import { RefreshCw, Trash2, CheckCircle2, AlertTriangle, History, X, Clock } from "lucide-react";
              ...
              {pendingActions.length === 0 ? (
                <div className="flex h-full flex-col items-center justify-center space-y-4 text-center">
                  ...
                  <p className="max-w-[200px] text-sm text-gray-500">
                    No pending actions in your offline queue.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  <div className="space-y-3">
                    {pendingActions.map((action) => (
                      <div
                        key={action.id}
                        className="space-y-3 rounded-xl border border-gray-100 bg-gray-50 p-4 dark:border-gray-600 dark:bg-gray-700/50"
                      >
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-500">
                          <span className="flex items-center gap-1">
                            <Clock size={12} /> {new Date(action.timestamp).toLocaleTimeString()}
                          </span>
                          <span className="flex items-center gap-1">
                            <RefreshCw size={12} /> Retry count: {action.retryCount}
                          </span>
                        </div>
                      </div>
                    ))}
            ...
            <div className="border-t border-gray-100 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-900/50">
              <div className="mb-4 flex items-center gap-3 rounded-lg border border-amber-100 bg-amber-50 p-3 dark:border-amber-800 dark:bg-amber-900/20">
                ...
                <p className="text-xs text-amber-700 dark:text-amber-400">
                  Actions will automatically sync when a stable connection is detected.
                </p>
              </div>
              <button
