 ...
 */
const Tooltip = ({ children, content, position = "top", delay = 300, className = "" }) => {
        ...
        createPortal(
          <AnimatePresence>
            <motion.div
              ...
              className={`rounded-lg bg-gray-900 px-3 py-1.5 text-xs font-medium whitespace-nowrap text-white shadow-xl dark:bg-gray-700 ${className}`}
            >
              {content}
              <div
                className={`absolute h-0 w-0 border-4 border-transparent ${arrowVariants[position]}`}
              />
            </motion.div>
          </AnimatePresence>,
          document.body
