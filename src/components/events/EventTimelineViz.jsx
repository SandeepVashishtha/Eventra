        ...
        {events.map((event, i) => (
          ...
          >
            ...
            <div className="pointer-events-none absolute top-8 w-48 rounded-xl border border-gray-100 bg-white p-4 opacity-0 shadow-lg transition-opacity group-hover:opacity-100 dark:border-gray-700 dark:bg-gray-800">
              <p className="mb-1 text-xs font-bold text-indigo-500">{new Date(event.date).toLocaleDateString()}</p>
              <h4 className="line-clamp-2 text-sm font-bold text-gray-900 dark:text-white">{event.title}</h4>
            </div>
          </motion.div>
        ))}
      </div>
