...
const AdminDashboard = () => {
  ...
  const loadWaitlist = useCallback((eventId) => {
    import("../../utils/waitlistUtils.js")
      .then(({ getEventWaitlist, getWaitlistAnalytics }) => {
        setWaitlistUsers(getEventWaitlist(eventId));

        setWaitlistAnalytics(getWaitlistAnalytics(eventId));
      })
      .catch(() => {
        setWaitlistUsers([]);
                        ...
                        <>
                          ...
                          <div className="animate-fadeIn absolute right-0 z-20 mt-1.5 w-36 rounded-xl border border-slate-200 bg-white py-1 shadow-lg dark:border-slate-800 dark:bg-slate-900">
                            <button
                              ...
                              className="dark:hover:bg-slate-850 w-full px-3 py-2 text-left text-xs font-semibold text-slate-700 transition hover:bg-slate-100 dark:text-slate-300"
                            >
                              Export as CSV
                            </button>
                            <button
                ...
                <table className="w-full border-collapse text-left text-xs">
                  ...
                  <tbody>
                    {waitlistUsers.map((w, index) => (
                      <tr
                        key={w.userId}
                        className="dark:hover:bg-slate-850/50 border-b hover:bg-slate-50/50 dark:border-gray-800/50"
                      >
