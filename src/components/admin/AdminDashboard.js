...
// Roles an admin is allowed to assign via the RBAC panel.
const ASSIGNABLE_ROLES = ["ATTENDEE", "ORGANIZER", "ADMIN"];

function RoleManagementModal({ targetUser, onClose, onAssign, saving }) {
  const [selectedRole, setSelectedRole] = useState(
    (targetUser?.roles && targetUser.roles[0]) || targetUser?.role || "ATTENDEE"
  );
  ...
  return (
    <div className="ad-modal-overlay" onClick={onClose}>
      <motion.div
        ...
        onClick={(e) => e.stopPropagation()}
      >
        <div className="ad-role-modal-head">
          <div>
            <h3 className="ad-modal-title" style={{ margin: 0 }}>
              Manage Role
            </h3>
            <p className="ad-role-modal-sub">
        ...
        <div className="ad-role-modal-body">
          <label className="ad-role-label" htmlFor="role-select">
            Assign role
          </label>
          <select
            id="role-select"
          ...
          <p className="ad-role-hint">
            Changing a user&apos;s role updates their platform permissions immediately.
          </p>
        </div>

        <div className="ad-modal-actions">
          <button className="ad-btn-ghost" onClick={onClose} disabled={saving}>
            Cancel
          </button>
          <button
            className="ad-btn-primary"
            onClick={() => onAssign(targetUser, selectedRole)}
            disabled={saving}
          >
...
const AdminDashboard = () => {
  ...
  const loadWaitlist = useCallback(
    (eventId) => {
      import("utils/waitlistUtils.js")
        .then(async ({ getEventWaitlist, getWaitlistAnalytics, syncWaitlistFromServer }) => {
          await syncWaitlistFromServer(eventId, user?.id);
          setWaitlistUsers(await getEventWaitlist(eventId, user?.id));

          setWaitlistAnalytics(await getWaitlistAnalytics(eventId, user?.id));
        })
        .catch(() => {
          setWaitlistUsers([]);
  ...
  const handleIncreaseCapacity = async () => {
    ...
    try {
      ...
      if (promotedCount > 0) {
        toast.success(
          `Capacity increased to ${newCap}. Promoted ${promotedCount} user(s) from waitlist!`
        );
      } else {
  ...
  const handleAssignRole = async (targetUser, role) => {
    setRoleSaving(true);
    try {
      await updateAdminUserRole(targetUser.id, role);
      toast.success(`Role updated to ${role} for ${targetUser.firstName} ${targetUser.lastName}.`);
                ...
                <motion.div variants={stagger} className="ad-stats-grid">
                  {statsLoading ? (
                    ...
                    <div className="ad-error-banner">
                      <AlertCircle size={16} /> Failed to load stats.{" "}
                      <button onClick={loadStats} className="ad-retry-btn">
                        Retry
                      </button>
                    </div>
                  ) : (
                            ...
                            <div key={ev.id} className="ad-list-item">
                              ...
                              <div style={{ flex: 1 }}>
                                ...
                                <p className="ad-list-sub">
                                  {ev.date} · {ev.participantCount ?? 0} participants
                                </p>
                              </div>
                ...
                <div className="ad-table-wrap">
                  {usersLoading ? (
                    ...
                    <div className="ad-error-banner">
                      ...
                      <button
                        onClick={() => loadUsers(usersPage, searchUser)}
                      ...
                      <tbody>
                        {users.length === 0 ? (
                          ...
                          users.map((u) => (
                            <tr
                              key={u.id}
                              className="transition-colors duration-200 hover:bg-slate-50 dark:hover:bg-slate-800/50"
                            >
                              <td>
                                ...
                                <div className="ad-action-btns">
                                  {hasPermission(PERMISSIONS.EDIT_USER) && (
                                    <button
                                      className="ad-icon-action"
                                      title="Edit role"
                                      aria-label={`Edit role for ${u.firstName} ${u.lastName}`}
                ...
                <div className="ad-table-wrap">
                  {eventsLoading ? (
                    ...
                    <div className="ad-error-banner">
                      ...
                      <button
                        onClick={() => loadEvents(eventsPage, searchEvent)}
                        className="ad-retry-btn"
                      >
                        Retry
                      </button>
                        ...
                        {events.length === 0 ? (
                          ...
                          events.map((ev) => (
                            ...
                            >
                              ...
                              <td>
                                <StatusBadge status={ev.type || "Event"} />
                              </td>
                      ...
                      [
                        {
                          label: "Total Users",
                          value: stats.totalUsers ?? 0,
                          sub: `${stats.activeUsers ?? 0} active · ${(stats.totalUsers ?? 0) - (stats.activeUsers ?? 0)} inactive`,
                          color: "#6366f1",
                          icon: <Users size={20} />,
                        },
                        {
                          label: "Events Hosted",
                          value: stats.totalEvents ?? 0,
                          sub: `${stats.upcoming ?? 0} upcoming · ${(stats.totalEvents ?? 0) - (stats.upcoming ?? 0)} completed`,
                          color: "#ec4899",
                          icon: <Calendar size={20} />,
                        },
                        {
                        ...
                        },
                        {
                          ...
                          value: Math.round(
                            (stats.totalParticipants ?? 0) / ((stats.totalEvents ?? 0) || 1)
                          ),
                          sub: "per event",
                          color: "#f59e0b",
        ...
        <footer className="ad-footer">
          ...
          <div className="ad-footer-content">
            ...
            <div className="ad-footer-links">
              ...
              <a
                href={`https://github.com/${ENV.GITHUB_REPO}`}
                target="_blank"
        ...
        <div className="ad-modal-overlay" onClick={() => setSelectedWaitlistEvent(null)}>
          ...
          >
            <div className="flex justify-between items-center mb-4 border-b pb-2">
              ...
              <button
                onClick={() => setSelectedWaitlistEvent(null)}
            ...
            {waitlistAnalytics && (
              ...
              >
                ...
                <p>Total Waitlisted: {waitlistAnalytics.totalWaitlisted}</p>

                <p>Waiting: {waitlistAnalytics.waiting}</p>

                <p>Promoted: {waitlistAnalytics.promoted}</p>

                <p>Removed: {waitlistAnalytics.removed}</p>

                <p>Promotion Rate: {waitlistAnalytics.promotionRate}%</p>

                <p>Average Wait Time: {waitlistAnalytics.averageWaitTime} hrs</p>
              </div>
            )}
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody>
                    {waitlistUsers.map((w, index) => (
                      <tr
                        key={w.userId}
                        className="border-b dark:border-gray-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-850/50"
                      >
