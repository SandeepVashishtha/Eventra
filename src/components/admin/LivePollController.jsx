...
export default function LivePollController({ eventId }) {
  ...
  // Attendee Voting State
  const [selectedOption, setSelectedOption] = useState("");
  const [hasVoted, setHasVoted] = useState(false);
  const [votingLoading, setVotingLoading] = useState(false);

  // Track votes in local storage to prevent duplicate votes
  useEffect(() => {
    if (activePoll) {
  ...
  const handleLaunchPoll = async (e) => {
    ...
    setSubmitting(true);
    try {
      await createPoll(newQuestion, pollType, filteredOptions);
      setNewQuestion("");
      setOptions(["", ""]);
      ...
      <div className="w-full flex flex-col gap-6 p-6 rounded-2xl bg-slate-900/30 backdrop-blur-xl border border-slate-800 shadow-premium-lg">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-bold text-slate-100 font-sans tracking-wide">Live Poll Control</h2>
          </div>
          {activePoll && (
            <span
        ...
        {activePoll ? (
          // Active Poll View for Moderators (Shows Results + Controls)
          <div className="flex flex-col gap-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button
                onClick={() => handleStatusChange("closed")} // Make sure we close old before starting new in actual flow, or just trigger creation screen
          ...
          // Create Poll Form for Moderators
    ...
    <div className="w-full flex flex-col gap-6 p-6 rounded-2xl bg-slate-900/30 backdrop-blur-xl border border-slate-800 shadow-premium-lg">
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-bold text-slate-100 font-sans tracking-wide">Live Poll</h2>
        </div>
      </div>
      ...
      {!activePoll ? (
        ...
        // Results View (already voted or poll paused/closed)
        <div className="flex flex-col gap-4">
