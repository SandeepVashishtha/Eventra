...
import { Zap, CheckCircle, Gift, Target, Flame, Star, Trophy, Sparkles, Timer } from "lucide-react";
...
const DAILY_QUESTS = [
  {
    id: "dq-1",
    title: "Join 1 Workshop",
    description: "Register for any workshop or webinar today.",
    icon: "📚",
    targetProgress: 1,
    rewardXP: 75,
  },
  {
    ...
    description: "Navigate to your profile page and review your stats.",
    icon: "👤",
    targetProgress: 1,
    rewardXP: 25,
  },
  {
    id: "dq-3",
    title: "Explore Events Page",
    description: "Browse the events listing and discover new opportunities.",
    icon: "🔍",
    targetProgress: 1,
    rewardXP: 50,
  },
...
const WEEKLY_QUESTS = [
  {
    id: "wq-1",
    title: "Register for 2 GSSoC Events",
    description: "Join two GSSoC-specialised events this week.",
    icon: "💻",
    targetProgress: 2,
    rewardXP: 200,
  },
  {
    ...
    description: "Log in and interact on 3 separate days.",
    icon: "🔥",
    targetProgress: 3,
    rewardXP: 300,
  },
  {
    ...
    description: "Fill out all profile fields to 100% completion.",
    icon: "🎯",
    targetProgress: 1,
    rewardXP: 150,
  },
  {
    id: "wq-4",
    title: "Attend 5 Community Meetups",
    description: "Register and attend five platform events.",
    icon: "🤝",
    targetProgress: 5,
    rewardXP: 400,
  },
  ...
  const renderQuest = (quest, isWeekly) => {
    ...
    return (
      <motion.div
        ...
        whileHover={{ y: -3 }}
        className={`relative cursor-default overflow-hidden rounded-2xl border p-5 backdrop-blur-xl transition-all duration-300 ${
          isClaimed
            ? "border-emerald-200/50 bg-emerald-50/40 dark:border-emerald-800/30 dark:bg-emerald-950/20"
            : isComplete
              ? "border-amber-300/50 bg-amber-50/40 shadow-[0_0_24px_rgba(245,158,11,0.08)] dark:border-amber-700/30 dark:bg-amber-950/15"
              : "border-slate-200/50 bg-white/60 hover:border-indigo-200 hover:shadow-lg dark:border-slate-800/40 dark:bg-slate-900/60 dark:hover:border-slate-700"
        } `}
      >
        {/* top row */}
        <div className="flex items-start justify-between gap-3">
          <span
            className={`shrink-0 rounded-xl p-2 text-2xl ${isClaimed ? "bg-emerald-100 dark:bg-emerald-900/40" : "bg-slate-100 dark:bg-slate-800/60"}`}
          >
            {quest.icon}
          </span>
    ...
    <section className="space-y-6" ref={confettiRef} style={{ position: "relative" }}>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-lg text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
            Complete missions to earn XP, climb levels, and unlock exclusive developer tokens.
            Quests refresh automatically.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-3 rounded-2xl border border-slate-200/50 bg-white/60 px-5 py-3 shadow-sm backdrop-blur-xl dark:border-slate-800/40 dark:bg-slate-900/60">
            ...
            <div>
              ...
              <p className="text-xl font-black tracking-tight text-slate-800 dark:text-slate-100">
                {state.lifetimeXP}
              </p>
            </div>
          </div>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 rounded-xl px-4 py-2.5 text-[11px] font-bold tracking-wider uppercase transition-all ${
                activeTab === tab.id
                  ? "bg-indigo-600 text-white shadow-md shadow-indigo-200 dark:shadow-none"
                  : "border border-slate-200/50 bg-white/60 text-slate-600 hover:bg-slate-50 dark:border-slate-800/40 dark:bg-slate-900/50 dark:text-slate-400 dark:hover:bg-slate-800/60"
              } `}
            >
              {tab.icon} {tab.label}
            </button>
        ...
        <div className="flex items-center gap-2 rounded-xl border border-slate-200/50 bg-white/60 px-4 py-2.5 shadow-sm backdrop-blur-xl dark:border-slate-800/40 dark:bg-slate-900/60">
          ...
          <div>
            <p className="text-[8px] leading-none font-black tracking-widest text-slate-400 uppercase">
              Refreshes in
            </p>
            <p
              className="mt-0.5 text-sm font-black tracking-tight text-slate-800 dark:text-slate-100"
      ...
      <div className="flex flex-col justify-between gap-3 rounded-2xl border border-slate-200/50 bg-white/60 p-4 backdrop-blur-xl sm:flex-row sm:items-center dark:border-slate-800/40 dark:bg-slate-900/60">
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200/50 sm:w-48 dark:bg-slate-800/40">
          <motion.div
            initial={{ width: 0 }}
            animate={{
              width: totalAvailableXP > 0 ? `${(claimedXP / totalAvailableXP) * 100}%` : "0%",
            }}
