...
const UserAvatar = ({ profilePicture, size = "sm" }) => {
  ...
  if (profilePicture) {
    return (
      <img
        src={profilePicture}
        alt="Profile"
        className={`${dimension} rounded-full object-cover ring-2 ring-primary/20`}
  ...
  return (
    <div className={`${dimension} rounded-full dark:bg-white/20 bg-gray-300 flex items-center justify-center`}>
      <UserIcon className={`${iconSize} text-gray-600 dark:text-white`} />
    </div>
  );
};

// --- Extracted: Desktop Auth Buttons (unauthenticated) ---
...
// --- Extracted: Desktop Profile Dropdown ---
      ...
      <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {secondaryLine && (
              <p className="text-xs text-gray-500 dark:text-gray-400 truncate">{secondaryLine}</p>
            )}
          </div>
        </div>
      </div>
...
// --- Extracted: Desktop Auth Section ---
const DesktopAuthSection = ({ user, isAuthenticated, primaryLine, secondaryLine, onLogoutClick, showProfileDropdown, setShowProfileDropdown }) => {
  if (!isAuthenticated()) {
    return <DesktopGuestButtons />;
  }
...
// --- Extracted: Dropdown Nav Item (Desktop) ---
const DropdownNavItem = ({ item, openDropdown, setOpenDropdown, isActive }) => {
  const location = useLocation();
...
// --- Extracted: Mobile Nav Item (with or without subItems) ---
const MobileNavItem = ({ item, openDropdown, setOpenDropdown, closeAllMenus }) => {
    ...
    return (
      <div>
        <button
          ...
          className={`flex items-center justify-between w-full px-3 py-2.5 rounded-lg transition-colors text-left text-sm font-medium ${
  ...
  return (
    <Link
      to={item.href}
      onClick={closeAllMenus}
      className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-lg transition-colors text-sm font-medium ${
        isActive ? activeClass : inactiveClass
      }`}
    >
      <span className="flex-shrink-0">{item.icon}</span>
...
// --- Extracted: Mobile Auth Section (authenticated) ---
...
// --- Extracted: Mobile Guest Buttons ---
...
const MobileDrawer = ({
  ...
  return (
    <div
      ...
      className={`fixed top-0 right-0 h-dvh w-[88vw] max-w-xs flex flex-col overflow-x-hidden overflow-y-auto shadow-2xl z-50 bg-white dark:bg-gray-900 transform transition-transform duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="px-2.5 py-3 border-t border-gray-200 dark:border-white/20 bg-white dark:bg-gray-900 space-y-2">
        ...
        <button
          ...
          className="flex items-center gap-3 w-full px-4 py-2.5 rounded-lg bg-black text-white hover:bg-zinc-800 transition-colors font-medium"
        >
...
// --- Extracted: Drawer keyboard trap ---
...
// --- Extracted: User display name logic ---
      ...
      >
        <div className="w-full flex items-center h-20 px-6 md:px-12 relative">
          ...
          <div className="hidden lg:flex items-center ml-auto z-20">
            <button
              onClick={toggleCursor}
              className="flex items-center gap-1 px-2 py-1 mr-3 text-s font-normal bg-black text-white rounded-md hover:bg-zinc-800 transition-all"
            >
